...
def get_budget_input():
    """获取用户输入的投资金额"""
    print("[输入] 请输入今日投资金额（元），直接回车默认100:")
    while True:
        try:
...
def main():
    ...
    print(f"[INFO] 分析日期: {report_date}")
    print()
    
    # 获取投资金额
    budget = get_budget_input()
    print(f"[INFO] 投资金额: {budget:.0f}元")
    print()
    ...
    report = AnalysisReport(
        ...
        notes=[
            ...
            "本分析仅供参考，不构成任何投注建议",
            f"<strong>投资策略</strong>: 第一注或第二注单独命中即可回本，第三注多串1博高倍数",
        ],
    )
    ...
    try:
        ...
        print(f"[INFO] 请手动打开报告: {output_file}")
